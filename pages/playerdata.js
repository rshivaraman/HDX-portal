...
export default function PlayersDashboard() {
  ...
  const handleSave = async () => {
    ...
    const { error } = editingPlayer
      ? await supabase.from('players').update(record).eq('id', editingPlayer.id)
  ...
  filtered.sort((a, b) => {
    ...
    return typeof aV === 'string'
      ? sortOrder === 'asc' ? aV.localeCompare(bV) : bV.localeCompare(aV)
      : sortOrder === 'asc' ? aV - bV : bV - aV;
  });
      ...
      <div className="max-w-7xl mx-auto backdrop-blur-md bg-black/40 p-6 rounded-2xl shadow-2xl border border-white/20 space-y-6">
        ...
        <div className="flex flex-wrap gap-3 justify-between items-center">
          <input
            ...
            className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 flex-1 min-w-[150px]"
          />
          ...
          <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600">
            ...
            <tbody>
              {paginated.map(p => (
                <tr key={p.id} className="border-t border-gray-700 hover:bg-gray-800/40 transition">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-1 rounded text-xs ${p.role === 'admin' ? 'bg-red-600' : 'bg-green-600'}`}>
                      {p.role || 'member'}
                    </span>
                  </td>
                  ...
                  <td className="p-3">{p.can_login ? '✔' : '✖'}</td>
                  <td className="p-3 flex flex-wrap justify-center gap-2">
                    {role === 'admin' ? (
                      <>
                        <button onClick={() => handleEdit(p)} className="bg-yellow-400 hover:bg-yellow-500 text-black px-3 py-1 rounded">
                          Edit
                        </button>
                        <button onClick={() => handleDelete(p.id)} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded">
                          Delete
                        </button>
                      </>
          ...
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 p-6 rounded-2xl w-full max-w-lg md:max-w-2xl shadow-xl border border-gray-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input name="troop_type" placeholder="Troop Type" value={form.troop_type || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800 border border-gray-600 text-white"/>
