    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-800 py-10 px-4 text-white">
      <div className="max-w-7xl mx-auto bg-black/40 p-6 rounded-2xl shadow-2xl border border-white/20 space-y-6">
        ...
        {role === 'admin' && (
          <div className="mb-6 p-6 border border-gray-700 rounded-2xl bg-black/30 shadow-inner space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <input name="event_name" value={form.event_name || ''} onChange={handleChange} placeholder="Event Name" className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white"/>
              <input type="date" name="event_date" value={form.event_date || ''} onChange={handleChange} placeholder="Event Date" className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white"/>
              <input type="number" name="min_participation" value={form.min_participation || 1} onChange={handleChange} placeholder="Min Participation" className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white"/>
              <input type="number" name="min_score" value={form.min_score || 1000} onChange={handleChange} placeholder="Min Score" className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white"/>
              <input name="season" value={form.season || ''} onChange={handleChange} placeholder="Season" className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white"/>
              <input name="description" value={form.description || ''} onChange={handleChange} placeholder="Description" className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white"/>
            </div>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          <input type="text" placeholder="Search by name..." value={search} onChange={e => setSearch(e.target.value)} className="flex-1 bg-gray-800 border border-gray-600 p-3 rounded-lg"/>
