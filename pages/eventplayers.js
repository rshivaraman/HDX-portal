...
// Displays numbers as 1,000 / 1,000,000 / 100,000,000
// Safely handles null, undefined, empty strings and strings
// ---------------------------------------------------------
...
export default function EventPlayersUI() {
  ...
  const [selectionView, setSelectionView] = useState('cards');
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const joined = (epData || []).map((ep) => ({
        ...ep,
        player:
          (playersData || []).find((p) => p.id === ep.player_id) || {},
        event:
          (eventsData || []).find((e) => e.id === ep.event_id) || {},
      }));
      ...
      // Initialise selectedPlayers from existing mappings
  ...
  useEffect(() => {
    ...
    if (selectedEventFilter) {
      result = result.filter(
        (ep) => ep.event_id === selectedEventFilter
      );
    }
  ...
  const handleSave = async () => {
    ...
    if (!form.event_id || !form.player_id) {
      return alert('Select event and player');
    }
  ...
  const handleInlineToggle = async (epId, current) => {
    ...
    const { error } = await supabase
      .from('event_players')
      .update({
        participation_choice: !current,
      })
      .eq('id', epId);

    if (error) {
      return alert('Error toggling: ' + error.message);
    }
  ...
  const refreshSelectedPlayersForEvent = async (eventId) => {
    try {
      ...
      (players || []).forEach((p) => {
        ...
        updatedMap[p.id] =
          !!found?.participation_choice;
      });

      setSelectedPlayers(updatedMap);
    } catch (err) {
      console.error(
        'Error refreshing player selections:',
        err
      );
    }
  };
  ...
  const openModal = async (forEventId) => {
    ...
    await refreshSelectedPlayersForEvent(
      targetEventId
    );
  ...
  useEffect(() => {
    ...
    refreshSelectedPlayersForEvent(
      modalEventId
    );
  }, [modalEventId]);
  ...
  const modalTotals = useMemo(() => {
    ...
    const selectedObjs = (players || []).filter(
      (p) => selectedIds.includes(p.id)
    );

    const totalBR = selectedObjs.reduce(
      (sum, p) =>
        sum + Number(p.battle_rating || 0),
      0
    );

    const totalMight = selectedObjs.reduce(
      (sum, p) =>
        sum + Number(p.might || 0),
      0
    );
  ...
  // --- Apply modal selections to event ---
  const applySelections = async () => {
    ...
    if (!modalEventId) {
      return alert(
        'Choose an event to apply selections'
      );
    }
    ...
    try {
      ...
      existing.forEach((r) => {
        existingByPlayer[r.player_id] = r;
      });
  ...
  const tableTotals = useMemo(() => {
    ...
    const totalBR = arr.reduce(
      (s, ep) =>
        s +
        Number(
          ep.player?.battle_rating || 0
        ),
      0
    );

    const totalMight = arr.reduce(
      (s, ep) =>
        s +
        Number(ep.player?.might || 0),
      0
    );
        ...
        <div className="mb-6 text-center text-white py-3 rounded-lg bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 shadow-lg border border-white/20">
          ...
          {selectedEventFilter ? (
            <>
              ...
              | Might:{' '}
              {formatNumber(
                tableTotals.totalMight
              )}
            </>
          ) : (
            <>
              <strong>
                All Events Combined (visible):
              </strong>{' '}
        ...
        {role === 'admin' && (
          <div className="bg-black/50 border border-gray-700 p-6 rounded-xl mb-8">
            ...
            {form.player_id && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                ...
                {[
                  ...
                  <div
                    key={stat.label}
                    className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 p-4 rounded-xl text-center shadow-md"
                  >
                    <div className="text-sm text-gray-400">
                      {stat.label}
                    </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={handleSave}
                className="bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded-lg font-medium shadow-md"
              >
                {editingId
                  ? 'Update Mapping'
                  : 'Add Mapping'}
              ...
              {editingId && (
                <button
                  ...
                  className="bg-gray-600 hover:bg-gray-700 px-5 py-2 rounded-lg font-medium shadow-md"
                >
                  Cancel
                </button>
              )}
