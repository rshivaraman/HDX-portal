      ...
      <div className="max-w-7xl mx-auto backdrop-blur-md bg-black/40 p-6 rounded-2xl shadow-2xl border border-white/20 space-y-6">
        ...
        <div className="mb-6 p-6 border border-gray-700 rounded-2xl bg-black/30 shadow-inner space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input name="full_name" placeholder="Full Name" value={form.full_name || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500"/>
            <input name="email" placeholder="Email" value={form.email || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500"/>
            <input name="country" placeholder="Country" value={form.country || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500"/>
            <input name="discord_id" placeholder="Discord ID" value={form.discord_id || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500"/>
            <input name="igg_id" placeholder="IGG ID" value={form.igg_id || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500"/>
            <input name="hero_name" placeholder="Hero Name" value={form.hero_name || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500"/>
            <input name="might" type="number" placeholder="Might" value={form.might || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500"/>
            <select name="troop_type" value={form.troop_type || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-blue-500">
            ...
            <select name="rank_id" value={form.rank_id || ''} onChange={handleChange} className="p-3 rounded-lg bg-gray-800/70 border border-gray-600 text-white focus:ring-2 focus:ring-purple-500">
              <option value="">Auto/Unset</option>
              {ranks.map(r => <option key={r.id} value={r.id}>{r.name} (min {r.min_might})</option>)}
            </select>
          </div>
          <div className="flex gap-3 mt-3">
            <button onClick={handleSave} className="bg-blue-600 hover:bg-blue-700 py-2 px-5 rounded-lg font-semibold shadow-md transition-all">{editingPlayer ? 'Update' : 'Add'}</button>
            {editingPlayer && <button onClick={handleCancel} className="bg-gray-500 hover:bg-gray-600 py-2 px-5 rounded-lg font-semibold shadow-md transition-all">Cancel</button>}
          </div>
        </div>
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-700">
          <table className="min-w-full text-sm text-gray-300">
            <thead className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">
              <tr>
                {['Name','Email','Country','IGG ID','Discord ID','Troop Type','Farm','Might','Rank'].map(col=>(
                  <th key={col} className="px-4 py-2 cursor-pointer" onClick={()=>{
                    const field = col==='Name'?'full_name': col==='Farm'?'farm_account': col==='Troop Type'?'troop_type': col==='Rank'?'rank_id': col.toLowerCase().replace(' ','_');
                    setSortField(field); setSortOrder(sortOrder==='asc'?'desc':'asc');
                  }}>
                    {col}
                  </th>
                ))}
              ...
              {displayed.map(player=>(
                <tr key={player.id} className="hover:bg-gray-800 border-t border-gray-700 transition-all">
                  ...
                  <td className="px-4 py-2">
                    <select value={player.rank_id||''} onChange={(e)=>handleRankChange(player.id,e.target.value||null)} className="bg-gray-800 border border-gray-600 p-1 rounded-lg text-white">
                      <option value="">Auto/Unset</option>
                      {ranks.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}
                    </select>
                  </td>
